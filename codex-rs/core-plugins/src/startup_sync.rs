...
fn git_command(git_binary: &str) -> Command {
    ...
    // pocket-codex: this git child is created with CREATE_NO_WINDOW. When codex
    // runs inside a GUI host process (no console) — e.g. the curated-plugins
    // sync at session start — an unflagged console-subsystem child would
    // otherwise flash a visible console window.
    #[cfg(windows)]
    {
