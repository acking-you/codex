...
impl PluginGitMode {
    ...
    pub(crate) fn command(self, git_binary: &Path) -> Command {
        ...
        // pocket-codex: plugin git output is always captured, never interactive.
        // When codex runs inside a GUI host process (no console), an unflagged
        // console-subsystem child opens a visible console window — e.g. a black
        // flash per repo during the curated-plugins sync at session start.
        // Applied here so every plugin git spawn inherits it.
        #[cfg(windows)]
        {
