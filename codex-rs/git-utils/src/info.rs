...
impl crate::FsmonitorProbeRunner for LocalFsmonitorProbeRunner<'_> {
    async fn run_probe(&mut self, args: &[&str]) -> Option<Vec<u8>> {
        ...
        // pocket-codex: CREATE_NO_WINDOW so this git metadata probe doesn't flash
        // a console window when codex runs inside a GUI host (no console).
        #[cfg(windows)]
...
async fn run_git_command_with_timeout_from(
    ...
    command
        ...
        .current_dir(cwd)
        .kill_on_drop(true);
    // pocket-codex: CREATE_NO_WINDOW so this internal git command doesn't flash a
    // console window when codex runs inside a GUI host (no console).
    #[cfg(windows)]
    command.creation_flags(0x08000000); // CREATE_NO_WINDOW
